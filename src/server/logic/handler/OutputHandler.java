...
public class OutputHandler {
	...
	public Output deleteItem(String input) {
        ...
        if(strArray.length!=2 || number!=true){
        		...
        		if(copynumber!=true){
        			output.setOutput("Your input should in this format:'ISBN,copynumber',Copynumber should be a number");
        			output.setState(DELETEITEM);
        		}else{
	...
	public Output borrow(String input) {
        ...
        if(strArray.length!=3 || email!=true){
        		...
        		if(ISBN!=true || copynumber!=true){
        			...
        			result=LoanTable.getInstance().createloan(userid, strArray[1], strArray[2], new Date(), userid + "17");
        			if(result.equals("success")){
        				output.setOutput("Success!");
	...
	public Output clerkLogin(String input) {
		...
		if(input.equalsIgnoreCase(Config.CLERK_PASSWORD)){
			output.setOutput("What can I do for you?Menu:Create User/Title/Item,Delete User/Title/Item,Monitor");
        	output.setState(CLERK);
		}else{
